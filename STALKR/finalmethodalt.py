'''
This used one of the equations from class to try and make a guess, it scored pretty much the same as my actual method
'''
class SNP:
    def __init__(self):
...
def getprob(locus, pop, status):
    ...
    if status == 'homo1':
        ...
        prob *= 2*snpdict[locus].zerofreq[pop] * snpdict[locus].onefreq[pop]
    return prob
...
with open("guesstestfreqs100k4popalt.txt", 'w') as wfile:
    for i in range(len(guessindividuals)):
