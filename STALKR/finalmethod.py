...
with open("glbtestindivids10k4pop.txt", 'r') as opfile:
    for line in opfile:
        individual = []
...
def guess(individual):
    ...
    initprobs.append(initafr) #we first initialize the individual to a population
    initprobs.append(initamr) #by examining a large portion of their genome and making a guess based on that
    ...
    while len(guessind) < len(individual):
        ...
        for snp in snpstocheck:
            locus = snp[0] #once we've initialized the person, we go by a cluster of SNPs at a time
            status = snp[1]
        ...
        for i in range(len(popprobs)):
            if i == pops.index(state):
                ...
                transprobs.append(popprobs[i] * .01) #incorporate a penalty for transitioning, since recomb is rare
...
with open("guesstestfreqs10k4pop.txt", 'w') as wfile:
    for i in range(len(guessindividuals)):
